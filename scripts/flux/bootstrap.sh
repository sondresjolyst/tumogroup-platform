...
# Install flux in tumogroup cluster.
...
# Create ConfigMap tumogroup-flux-config which will provide values for flux deployments
echo "Creating \"tumogroup-flux-config\"..."
...
printf "    Deploy \"tumogroup-flux-config\" configmap in flux-system namespace... "
cat <<EOF >tumogroup-flux-config.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
kubectl apply -f tumogroup-flux-config.yaml 2>&1 >/dev/null
rm tumogroup-flux-config.yaml
printf "Done.\n"
